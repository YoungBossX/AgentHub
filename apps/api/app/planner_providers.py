...
class ClaudeCliPlannerProvider:
    ...
    def _build_command(self, planner_input: dict[str, Any]) -> list[str]:
        prompt = (
            "You are AgentHub's llm_v1 planning engine. Return ONLY one JSON "
            "object. Do not include Markdown fences, analysis text, comments, "
            "or a second JSON object. The JSON must match this PlannerResponse "
            "contract exactly enough to validate: top-level fields planId, "
            ...
            "not task titles. Do not edit files, run commands, deploy, or call "
            "external services. Use only the provided target registry, roles, "
