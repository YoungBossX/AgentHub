...
existing TaskRun, TaskRunEvent, artifact, preview, and deployment evidence.
AgentHub now exposes safe read-only diagnostics for a TaskRun at
`/task-runs/{task_run_id}/diagnostics` and a Session summary at
`/sessions/{session_id}/run-diagnostics-summary`.

The projection classifies common provider, adapter, queue, lock, worktree,
validation, approval, preview, deployment, artifact collection, and unknown
failures; builds a run timeline; summarizes provider/queue/lock/preview/deploy
health; and returns next-step suggestions. Diagnostic metadata is redacted and
truncated before API response serialization.

This backend slice does not change execution semantics, add adapters, add
WebSocket/Docker/PR/external IM behavior, or implement the V2.7 frontend UI.
Frontend integration remains pending because existing
`apps/web/src/components/**` files had concurrent uncommitted changes during
the backend implementation.

## V2.1 Durable Run Engine Status

V2.1 completed on 2026-06-09. AgentHub now has a durable TaskRun execution
boundary with `run_engine.py`, a `RunSupervisor` registry, shared scheduling
entrypoints, worker claim / lease / heartbeat, max runtime timeout handling,
supervisor-backed interrupt attempts, stale recovery, completed-run finalizer
boundaries, and mission trace durable run evidence.

The implementation preserves existing adapters, Scheduler, Target Registry,
PlanValidator, worktree/diff/review/preview/deploy evidence, and ScriptedMock
fallback behavior. A separate long-lived worker process, provider circuit
breaker, explicit DB target lock, full policy engine, and transactional
rollback remain deferred to later Reliability V2 phases.

## P24 Status

### P24-2 Deployment Request And Status Evidence

P24-2 added honest deployment request/status evidence on 2026-06-08. AgentHub
can now persist manual external handoff deployment cards and blocked external
provider cards for unavailable providers such as Vercel, Netlify, and custom
static host. These cards record provider metadata, source preview linkage, logs,
and status history without claiming third-party success.

Existing mock and local staging deployment behavior remains compatible. Local
staging still records ready/failed status based on real build/serve evidence.

### P24-1 Deployment Provider Registry

P24-1 added a safe deployment provider registry on 2026-06-08. AgentHub now
exposes local staging, manual external handoff, Vercel, Netlify, and custom
static host deployment provider metadata through `/deployment-providers`.

The registry reports provider id, display name, provider type, supported
artifact kinds, supported target types, auth/config status, availability,
approval requirement, and secret environment variable names. It never returns
raw secret values and does not execute third-party deploys.

## P23 Status

### P23-6 Artifact References In Chat Context

P23-6 added message-level artifact context on 2026-06-08. Follow-up messages can
now carry selected artifact id, artifact version id, safe summary, selected
text/section metadata, and quoted message context. AgentHub stores this as
`Message.context_json` and merges it into the session context pack used by the
Planner and coding agents.

Artifact references now support markdown, text, and code workbench artifacts in
addition to diff, review, preview, and deployment evidence. Metadata is redacted
before provider-visible context is built. Executable follow-up changes still go
through Conversation Router / Planner, PlanValidator, Scheduler, and coding
agents.

### P23-5 Artifact Editor And Version History UI

P23-5 added lightweight artifact editor controls on 2026-06-08. Editable
workbench artifacts now show edit, save, and cancel controls in the right-side
artifact panel. Saving calls the Artifact Workbench edit API to create a new
artifact version and refreshes workbench metadata; it does not directly write to
repository files.

The panel shows version history with version number, content hash, and summary.
Unknown or unsupported artifacts remain read-only and do not expose edit/save
controls.

### P23-4 Artifact Workbench UI

P23-4 added Artifact Workbench UI support on 2026-06-08. The right-side artifact
panel now loads session workbench metadata, merges it with existing
diff/review/preview/deploy evidence, and shows a dedicated document/workbench
rail for artifacts not already covered by evidence cards.

The panel renders markdown/text/code-like artifact versions with content,
content hashes, renderer kind, editability, and version history. Unknown
artifacts use a safe metadata fallback. The chat workspace remains simple:
details stay in the right-side artifact panel.

### P23-3 Safe Artifact Editing API

P23-3 added a safe Artifact Workbench edit API on 2026-06-08. Editable
markdown/text/code artifacts can now be saved as new ArtifactVersion records
with parent version id, content, content hash, editor source, and summary.

Artifact edits do not write to repository files, create git diffs, or mutate
TaskRun evidence. Unsupported artifacts such as diff, review, deployment,
binary, or unknown types are rejected honestly. Existing SQLite databases get
compatibility columns for artifact version edit metadata.

### P23-2 Artifact Workbench API

P23-2 added backend Artifact Workbench API routes on 2026-06-08. AgentHub can
now list session artifacts for a workbench, load artifact renderer metadata,
load workbench-safe version metadata, expose stable content hashes, and return
safe redacted metadata without exposing protected host paths or secrets.

P23-2 keeps the existing `/artifacts/{artifact_id}/versions` response intact for
backward compatibility. Artifact editing and workbench UI are still deferred to
later P23 tasks.

### P23-1 Artifact Version Metadata Foundation

P23-1 added artifact workbench metadata helpers on 2026-06-08. AgentHub can now
classify artifact renderer kinds for web previews, markdown/text documents, code
snippets, diff, review, deployment, and unknown fallback artifacts. It also
derives editable flags, redacted safe metadata, stable content hashes, and
version metadata from existing Artifact / ArtifactVersion storage.

Artifact editing is not implemented in P23-1. The helper only exposes safe
metadata foundations for later workbench APIs and UI.

## P22 Status

### P22 Freeze

P22 completed implementation and entered freeze review on 2026-06-08. The phase
added the Agent Directory backend/API, compatibility policy, safe metadata-only
drafts, runtime config compatibility enforcement, Agent Directory settings UI,
and safe draft UI.

P22 preserves the separation between Planner runtime providers and coding
agents. It does not add arbitrary shell-command agents, OpenCode, a provider
marketplace, new adapters, or executable custom draft agents.

### P22-6 Draft Agent UI

P22-6 added a safe custom agent draft form to the Agent Directory settings page
on 2026-06-08. The form supports save and cancel controls for metadata-only
drafts. New drafts use the safe `scripted_mock` review-only path, declare
review/read-only modes, stay `safeForWrite=false`, and are displayed as
`draft_only` / unavailable / incompatible until validated.

The UI does not expose arbitrary shell command, unsafe tool permission, or
unrestricted filesystem controls. Backend validation errors are shown honestly
without creating an executable agent.

### P22-5 Agent Directory UI

P22-5 added a dedicated Agent Directory settings page on 2026-06-08 at
`/settings/agents`. The page loads the workspace Agent Directory API and shows
contact-style cards with provider id, adapter type, capabilities, supported
targets, supported modes, safety flags, availability, runtime-selected roles,
and compatibility reasons/warnings.

The directory page supports local filters by role, provider, capability, target,
and status. Filtering does not mutate runtime config. The chat sidebar now links
to the Agent Directory settings page while keeping detailed provider and
compatibility configuration out of the chat workspace.

### P22-4 Runtime Config Compatibility Integration

P22-4 connected runtime config validation to Agent Directory compatibility on
2026-06-08. Frontend, Backend, and Review runtime role selections now reuse the
same compatibility checks that power the Agent Directory, including provider
availability, adapter alignment, role support, mode support, capability support,
write/review safety, and draft-only rejection.

Planner runtime validation remains separate from coding-agent compatibility:
Planner provider configuration is checked against Planner provider metadata and
review-safe planner profile requirements, preserving the P17 separation between
Conversation/Planner LLM and coding agents. Invalid runtime selections fail
honestly and validation-only requests do not persist candidate config.

### P22-3 Safe Draft Agent Management

P22-3 verified safe custom agent draft behavior on 2026-06-08. Draft creation is
metadata-only, workspace-scoped, and constrained to review/read-only/debug modes.
Drafts reject write-enabled profiles, arbitrary shell commands, unsafe tool
permissions, unrestricted filesystem access, unknown providers, and adapter
mismatches.

Drafts appear in Agent Profile and Agent Directory metadata, but remain
`draft_only`, unavailable, and incompatible for execution until a later validated
execution policy exists. Failed unsafe draft requests do not persist drafts.

### P22-2 Agent Compatibility Policy

P22-2 added explicit Agent Directory compatibility policy on 2026-06-08. AgentHub
now checks whether a selected profile/provider can satisfy a requested role,
adapter, target, mode, required capabilities, provider availability, and
write/review safety flags.

Directory entries include compatibility metadata, and the workspace-scoped
compatibility endpoint:

- `POST /workspaces/{workspace_id}/agent-directory/check-compatibility`

returns user-readable reasons and warnings without exposing secrets. Draft-only
profiles remain incompatible for write execution until validated.

### P22-1 Agent Directory Backend

P22-1 added the backend Agent Directory foundation on 2026-06-08. AgentHub now
has a derived workspace-scoped Agent Directory endpoint that combines existing
AgentProfile registry entries, ProviderConfig metadata, AgentRuntimeConfig
selection, and AgentProfileDraft metadata.

The directory includes built-in and draft entries with provider id, adapter type,
capability tags, supported targets, supported modes, safety flags, status, auth
status, availability, and `runtimeSelectedForRoles`. Draft entries remain
unavailable for execution by default. Directory responses are metadata-only and
do not expose raw API keys, tokens, credentials, or secret fields.

## P21 Status

### P21 Freeze

P21 completed and passed freeze validation on 2026-06-08. Full validation
passed:
...
P21 remains bounded to PMO coordination and explanation. It does not add
arbitrary plan editing, automatic conflict merge, auto-stash/discard behavior,
new coding adapters, or multi-user approval workflow.

### P21-5 PMO Workspace UI

P21-5 added PMO coordination UI on 2026-06-08. Task cards now show
`planJson.pmoDecision` state, actor, reason, next-action summary, and controls
to approve, reject, or request clarification for pending reviewed plans. These
controls call the narrow PMO plan decision endpoints added in P21-2 and refresh
the task timeline.

The mission/context panel now shows a compact PMO readiness summary derived from
current tasks: ready, waiting, blocked, and up to three recommended next action
labels. This keeps the chat surface lightweight while making Main Agent / PMO
coordination visible.

### P21-4 PMO Mission Trace Evidence

P21-4 added redacted PMO coordination evidence to mission trace responses on
2026-06-08. Mission trace now includes `pmoEvidence` entries for PMO plan
decisions, scheduler blockers, planner fallbacks, and conflict summaries. The
evidence is derived from existing task plan metadata and blocker state; it does
not claim execution success.

PMO evidence redacts secret-like values and protected host paths such as `.env`,
`.git`, `node_modules`, `.venv`, and `secrets` paths while preserving safe
relative application paths needed for inspection.

### P21-3 PMO Graph Readiness And Next Actions

P21-3 added PMO-derived mission trace coordination metadata on 2026-06-08.
Mission trace responses now include `taskGraphReadiness` groups for ready,
waiting dependency, waiting target lock, blocked, running, and complete tasks.
These groups are derived from existing task status and scheduler metadata; they
are not persisted separately.

Mission trace `nextActions` now includes auditable PMO guidance for starting
ready tasks, inspecting blockers, approving/rejecting/requesting clarification
for PMO-reviewed plans, retrying failed tasks, choosing explicit fallback, and
approving or denying waiting TaskRuns. These next actions are guidance only:
actual execution still goes through existing TaskRun, retry, fallback, and
approval endpoints.

### P21-2 PMO Plan Decision Actions

P21-2 added narrow PMO plan decision API actions on 2026-06-08:
...
The endpoints accept only an optional `reason`. They update
`planJson.pmoDecision` through the PMO metadata helper and reject raw plan
mutation fields such as client-supplied `planJson`. Approval keeps the task
eligible for existing Scheduler and TaskRun creation paths; it does not start a
TaskRun directly and does not bypass PlanValidator or Target Registry behavior.
Reject and clarification mark the task blocked and do not create TaskRuns.

### P21-1 PMO Decision Metadata Foundation

P21-1 added the first Main Agent / PMO foundation on 2026-06-08. AgentHub now
has stable task `plan_json` metadata helpers for `pmoDecision` states:
`pending_review`, `approved`, `rejected`, and `clarification_needed`.

The decision metadata includes schema version, state, actor, reason, timestamps,
and a next-action summary. It is stored in existing task plan JSON and exposed
through mission trace task entries, so no new persistence table was introduced.
Unsupported raw plan mutation fields such as client-supplied `planJson` are
rejected by the PMO helper boundary.

P21-1 does not add approve/reject/clarification API endpoints, does not start or
stop TaskRuns, and does not change Scheduler, PlanValidator, Target Registry, or
Guardrails behavior. Those are later P21 tasks.

## P20 Status

### P20-5 Artifact Navigation Workbench

P20-5 improved artifact navigation on 2026-06-08. Task timeline artifacts are
now presented as user-facing evidence cards with Chinese action labels for
viewing Diff, review, preview, deployment, creating review/deploy cards, and
using artifacts as follow-up context. The right-side artifact panel empty state
now explains that Diff, Review, Preview, and Deployment evidence can be selected
from the task timeline. P20-5 does not add artifact editing.

### P20-4 Agent, Target, Memory, And Evidence Summary

P20-4 improved the mission/context panel on 2026-06-08. The panel now shows the
current goal, active role agents, selected frontend/backend targets, shortened
memory snapshot id, latest evidence status, last successful adapter, recent
changed files, and deployment provider/status. It derives this from existing
session and ledger responses; no API/schema change was needed.

### P20-3 Read-only Plan Review Surface

P20-3 completed the read-only plan review surface on 2026-06-08. Task cards
already showed planner rationale, role, target, planned files, acceptance
criteria, validation expectations, and read-only status. P20-3 added
user-readable planner evidence for fallback source/reason, provider id,
validation result, error code, and safe error summary. No plan mutation controls
were added.

### P20-2 Conversation Mode And Context Display

P20-2 added local Direct/Group conversation mode display on 2026-06-07. The
mode control is UI framing only and does not change backend routing, Scheduler,
PlanValidator, or task creation behavior. The message composer now shows
quoted messages and selected artifacts as pending context before send, with a
single clear-context control.

### P20-1 Session Navigation Polish

P20-1 added daily workspace session filtering on 2026-06-07. The left sidebar
now includes a session search field, filtered session count, selected-session
task focus summary, and an empty search state. Filtering is local UI behavior:
it does not clear the selected session, does not create messages, and does not
create Tasks or TaskRuns.

## Local Folder Picker Target Registration

As of 2026-06-07, runtime workspace settings can register external frontend or
backend targets from a local folder picker instead of requiring users to type
allowed paths by hand. The picker exposes safe start locations such as Desktop,
Documents, and the workspace-adjacent folder, filters hidden/protected folders,
and supports parent/child folder navigation.

Selected-folder registration uses `allowedPaths: ["*"]` so the chosen project
folder becomes the write scope, while protected paths such as `.git`,
`node_modules`, `.env`, virtual environments, build output, and secrets remain
denied by target policy and guardrails. The runtime settings UI lets users
choose whether the selected folder should become a frontend or backend target.

## P18c Status

### P18c Live Memory Compliance Library App Freeze

P18c completed on 2026-06-07.

The final live memory compliance smoke used the business-only prompt:

```text
帮我在桌面开发一个简单的图书管理系统。有登录页面，初始账户和密码是 18088888888 / 888888。登录后进入管理页面，只需要有图书管理功能：加入图书、删除图书、修改图书、查询图书。
```

The prompt did not restate the long-term memory rules. AgentHub routed the task
to the external frontend target `external-p18c-library-app`, and a real
CodexAdapter run created a Vite + React + TypeScript Library Management App
under `/Users/luotianhang/Desktop/agenthub-rehearsals/p18c-library-app`.

Final evidence:

- Session: `9dd8ff1d-be15-4114-b47c-4d3664838e53`
- Task: `11dda813-bfb6-4130-b410-08fe90a74c95`
...
- Diff: `7a6dd596-3d9d-4810-b1fb-c7a66d3ac67f`
- Review: `a9ac0c07-547a-4bbd-b2a1-6aa867524304`
- Passing check/test/build evidence:
  `801bad3c-a9f7-479f-8e09-59e1eaadf9e8`,
  `c1a86bc8-7eba-48c5-a377-c65a0f488941`, and
  `34d4070e-cd5d-450f-bd79-ca449f6b9511`
- Preview: `53d8cd5e-0322-449e-a10d-9456633a23e2`, healthy during smoke
- Staging: `20d26ddf-736e-49d6-abe0-25e7b0b843db`, ready during smoke

The generated app includes login with `18088888888 / 888888`, a management page,
add/delete/edit/search book flows, and localStorage persistence. No backend or
database was created.

Compliance metrics:

- Preference Recall Rate: `1.0`
- Project Memory Recall Rate: `0.8333`
- Cross-Agent Consistency Rate: `1.0`
- Snapshot Consistency Rate: `1.0`
- Change-log Missing Rate: `0.0`
- Target Boundary Violation Count: `0`
- Persistence Memory Violation Count: `0`
- Provider Evidence Violation Count: `0`
- Task Success Delta: unknown / inconclusive because the control was a
  deterministic dry-run, not a comparable live no-memory run

Follow-up was required and recorded honestly. Initial check/test/build failed
because the external Vite project had no `node_modules`; after the user
explicitly allowed dependency installation for the external P18c project,
`pnpm install` was run only there and check/test/build passed. Preview/staging
also exposed an external target preview-root gap, fixed by commit `15e4702`.

The configured DeepSeek planner did not run because `DEEPSEEK_API_KEY` was not
available to the API process environment; the task used the audited external
target fallback planner. This limitation is recorded in
`docs/p18c-freeze-review.md`.

### P18c Memory Rule Simplification

P18c memory rules were simplified on 2026-06-07 after the first live planner
attempt treated the Library Management App as a broader frontend/backend plan
instead of a localStorage-only app. The active rules under test are now written
as general Chinese long-term memory:
...
The P18c memory seeding helper archives obsolete active P18c rules from the
same source before creating or reusing the current rules, so new snapshots do
not mix the previous narrow English demo wording with the new general rules.

### P18c-3 Live Library App Execution Blocker

P18c-3 was retried on 2026-06-07 after simplifying the memory rules.
...
- Adapter: `claude_code`
- Provider: `local-claude-code-cli`
- Final state: `failed`
- Error: `CLAUDE_CODE_EXIT_ERROR`, `API Error: Unable to connect to API (ConnectionRefused)`
- Artifacts: none

The simplified memory rules improved routing enough to create an external
frontend task and start a real ClaudeCodeAdapter TaskRun. The live app was not
implemented because Claude Code could not connect to its API runtime. ScriptedMock
was not used to claim live memory compliance.
...
P18b completed on 2026-06-05.

AgentHub now has a bounded memory effectiveness rehearsal layer on top of the
P18 memory control plane. It defines deterministic, realistic scenarios for:

- Chinese user preference recall;
- project-rule compliance for change-log expectations;
- stale/archived memory exclusion;
- prompt-injection blocking for untrusted external suggestions.

The P18b evaluator compares memory-minimal control context against
memory-enabled treatment context, records memory snapshot IDs, computes
Preference Recall Rate, Memory Precision@5, Stale Memory Injection Count,
Prompt Injection Write Block Rate, Snapshot Consistency Rate, and explicitly
marks Task Success Delta / Change-log Missing Rate as unknown when comparable
live task evidence is absent.

The P18b smoke result showed deterministic improvement for all four built-in
scenarios without stale-memory injection. No live Planner or coding-agent
success is claimed because `OPENAI_API_KEY`, `DEEPSEEK_API_KEY`, `MIMO_API_KEY`,
and `ANTHROPIC_API_KEY` were unset in the process environment. Details are
recorded in `docs/p18b-freeze-review.md`.

P18b does not add automatic long-term learning, embeddings/RRF/graph retrieval,
provider marketplace, production deploy, or any guardrail bypass. Memory remains
guidance only; Target Registry, PlanValidator, Guardrails, runtime config, and
scheduler policy remain the enforcement boundaries.

## P17 Status

### Post-P17c UI And Workspace Settings Updates

As of 2026-06-02, the working tree includes UI and settings updates that go
beyond the frozen P17c OpenSpec scope. Treat them as current implementation
facts once committed, but do not assume they represent a new full platform
scope.

The chat workspace has been visually refreshed into a simpler three-panel
workbench. Detailed configuration has moved out of the primary chat page and
into settings routes:

- `/settings/contacts` shows the current built-in Agent contact list and local
  Direct/Group visual mode controls;
- `/settings/runtime` contains Agent runtime provider settings plus workspace
  target settings;
- `/settings/other` is a reserved placeholder route.

The runtime settings page now also exposes workspace configuration that was
previously backend-only:

- workspace target list loading through `GET /workspaces/{workspaceId}/targets`;
- external project analysis through
  `POST /workspaces/{workspaceId}/external-targets/analyze`;
- external project registration through
  `POST /workspaces/{workspaceId}/external-targets`;
- per-session frontend/backend target selection through
  `PATCH /sessions/{sessionId}/target-selection`.

This makes P9 external workspace mode and P16 runtime configuration visible to
the user, but it does not create multi-user workspace sharing, cloud project
import, arbitrary filesystem access, or production deployment. The product
boundary remains: local single-user Agent Coding Workspace with IM-style
interaction.
...
P12-10 completed on 2026-05-26.

Result: P12 is ready to freeze as Platform Core Consolidation.

The freeze rehearsal used a fresh SQLite database
`sqlite:///data/p12-freeze-rehearsal-3.sqlite3` and a new session worktree. It
verified the consolidated local demo path:
...
- first frontend run ID: `89dd57d7-c003-4a5e-b9fa-a4ca6ed0e326`;
...
- first preview ID/status: `9e578e93-ced3-41ed-a8c2-5f6752f4b455`,
  `healthy`;
- first local staging deployment ID/status:
...
- follow-up preview ID/status: `049ca7aa-c7e0-46a9-ba06-ffc81d77b347`,
  `healthy`;
- follow-up local staging deployment ID/status:
  `b7e52a37-09a6-4246-9f87-b406f02a251a`, `ready`.

No fresh Claude Code or Codex mutation was run during P12-10. The rehearsal
used ScriptedMockAdapter for deterministic local evidence and preserved the
P6-P11 baselines through full validation.

Current caveats:

- the default local SQLite database may need reset/migration if it predates
  P11 external-target deploy columns;
- worktree setup symlinks for `node_modules` should be ignored locally so they
  do not trip dirty-worktree conflict detection;
- local staging URLs were healthy at creation but were not kept long-lived
  after the one-shot rehearsal process exited;
- handoff evidence was generated explicitly through the P12 handoff service
  after diff/review/deploy artifacts existed so artifact refs were present.

See `docs/p12-freeze-review.md` for full evidence and validation results.
...
P19 is in progress and has completed the routing hardening core through the
novel-app regression set.

Current P19 baseline:

- API planner providers and Claude CLI planner use one canonical planner prompt
  contract.
- The canonical prompt requires safe software build / implement / create /
  develop / modify requests to return `task_plan` with a complete `planDraft`.
- Normal greetings and capability questions remain `assistant_reply` and do not
  create tasks.
- Clarification, refusal, and approval-required outcomes remain non-executing.
- Safe external frontend coding requests misclassified as `assistant_reply`
  fall back to audited deterministic frontend tasks.
- Fallback tasks record `plannerSource=fallback`, `fallbackReason`,
  `llmOutcomeType`, provider metadata, validation result, safe error summary,
  and created task ids in `plannerEvidence`.
- Invalid LLM `task_plan` outputs record validation failure evidence before any
  deterministic fallback task is created.
- Fixed modules such as login, todo, notes, mini CRM, and Breakout remain
  regression baselines, not capability limits.
- P18c Library Management App routing now creates a target-scoped frontend task
  when a prepared external frontend target exists.
- Missing desktop/external targets ask for target setup instead of writing
  arbitrary host paths.

P19 has not yet completed final freeze review. P18c live execution remains
paused until P19 freeze evidence confirms planner routing is no longer the
blocker.

### P24 Deployment Provider Status Cards

P24 is in progress. Completed so far:

- P24-1 added a deployment provider registry for local staging, manual external
  handoff, and unavailable external static providers.
- P24-2 added honest deployment status evidence for ready, blocked, failed, and
  handoff outcomes without claiming third-party deploy success.
- P24-3 upgraded the deployment card UI to show provider type, environment,
  source preview/diff/review references, logs, status history, and an open-URL
  action only when a URL exists.
- P24-4 lets deployment cards participate in artifact context follow-ups with
  safe provider/source/log metadata and secret-like text redaction.
- P24-5 freeze review passed a bounded rehearsal for a local staging deploy
  card and a blocked Vercel card. Third-party provider execution remains
  deferred; external cards are blocked or handoff evidence only.

P24 is ready to freeze after the freeze commit.

### P25 Context Control And Secondary Interaction

P25 is in progress. P25-1 added backend context item normalization for explicit
`contextItems`, legacy selected artifact context, and quoted message context.
Context items now have deterministic limits and redaction for protected paths,
secret-like strings, selected text, notes, and metadata before later planner
handoff integration.

P25-2 connected normalized context items to session context packs, canonical /
provider-visible context, planner evidence, and mission trace. Context-aware
follow-ups now carry auditable `contextHandoff` metadata without granting any
new execution permissions.

P25-3 upgraded the composer to a compact multi-item Context Tray. Users can now
stage multiple message/artifact/deployment context items, remove them, reorder
them, clear the tray, and send a payload that includes normalized
`contextItems` plus legacy compatibility fields.

P25-4 added secondary interaction actions for artifact and deployment cards:
ask about this, revise with this, and send to Agent. These actions only stage
context and prefill the composer; pure chat with context still produces a chat
reply without TaskRuns, while executable follow-ups continue through the normal
Router / Planner path after send.

P25-5 freeze review passed a bounded rehearsal combining deployment, diff,
workbench artifact version, and quoted message context. P25 is ready to freeze
after the freeze commit.

### Runtime Settings Provider Override Save Fix

Runtime settings now allow users to combine a built-in role profile such as
`Frontend Agent` with a selected runtime coding provider such as `Claude Code
CLI`. Backend validation treats the profile as the role/capability template and
uses the selected provider as the source of truth for runtime adapter/provider
execution. Payloads where the selected adapter does not match the selected
provider remain invalid.
